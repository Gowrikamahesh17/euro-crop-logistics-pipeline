...
window.PIPELINE_STATE = {
  ...
  nodes: {
    ...
    target_efficiency: { status: "done",        note: "LinearRegression baseline + LGBMRegressor tuned via RandomizedSearchCV." },
    target_quality:    { status: "done",        note: "LinearRegression baseline + LGBMRegressor tuned via RandomizedSearchCV." },
    target_croptype:   { status: "done",        note: "Vehicle_Type classification: LogisticRegression baseline + XGBClassifier tuned via RandomizedSearchCV (label-encoded). Accuracy ~45% — flagged honestly in dashboard as weak, not hidden." },
